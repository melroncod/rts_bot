...
│   ├── admin_catalog.py  # админ-команды /hide /show /price
...
6. **🎲 Случайный чай**: кнопка в главном меню или команда `/random` — выдаёт случайный
   **чай** из каталога карточкой; кнопка «🎲 Ещё раз» роллит заново (не повторяя текущий).
   Не-чайные категории (посуда, чайные духи) в ролле не участвуют — список исключений
   задаётся константой `NON_TEA_CATEGORIES` в `bot/bot.py`.
...
## Администрирование из Telegram

Команды работают только для Telegram ID из переменной `ADMIN` (остальным бот не отвечает)
и видны в меню команд только у админов.

| Команда | Что делает |
|---|---|
| `/hide` | Список товаров в продаже с их ID |
| `/hide 12` или `/hide лунцзин` | Скрыть товар (закончился) — по ID или части названия |
| `/show` | Список скрытых товаров с их ID |
| `/show 12` или `/show лунцзин` | Вернуть товар в продажу |
| `/price 12 2600` или `/price лунцзин 2600` | Изменить цену (можно `349,50`, `2600₽`) |
| `!message <user_id> <текст>` | Написать пользователю от имени бота |

- Товар не удаляется, а получает `is_active = false`: описание и фото сохраняются.
- Скрытый товар сразу пропадает из каталога, поиска, рандома, корзин и заказов;
  меню категорий обновляется сразу.
- Если по названию нашлось несколько товаров, бот покажет их ID и ничего не изменит.
- Число в аргументе всегда трактуется как ID.
