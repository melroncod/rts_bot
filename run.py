...
async def main():
    # reload управляется переменной окружения (по умолчанию выключен — для продакшена).
    # Примечание: uvicorn reload не работает при программном запуске через server.serve().
